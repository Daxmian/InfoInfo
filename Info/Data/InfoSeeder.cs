...
namespace Info.Data
{
    public class InfoSeeder
    {
        ...
        private static void SeedCategoris(ApplicationDbContext dbContext)
        {
             if (!dbContext.Categories.Any())
             {
                                var kat = new List<Category>
                 {
                                    ...
                                    new Category { Name = "Wiadomości", Active = true,
                Display=true, Icon="chat-left-text", Description="Najświeższe wiadomości i informacje z dziedziny informatyki. Coś dla programistów i zwykłych użytkowników komputerów, tabletów oraz smartfonów." },
            
                                    new Category { Name = "Artykuły", Active = true,
                Display=true, Icon="journal-richtext", Description="Artykuły w naszym serwisie pisane są przez wybitnych znawców tematu, którzy z olbrzymią przenikliwością zgłębiają każdy temat."},
             
                                    new Category { Name = "Testy", Active = true,
                Display=true, Icon="speedometer", Description="Nasze laboratorium testuje dla Was najnowszy sprzęt, poddając go elektronicznym torturom i wyciskając siódme poty elektronów."},
             
                                    new Category { Name = "Porady", Active = true,
                Display=true, Icon="life-preserver", Description="Jeżeli wciąż masz problemy z obsługą komputerów lub chcesz pracować efektywnie zajrzyj do sekcji z poradami dla adminów i laików." },
        ...
        private static void SeedTexts(ApplicationDbContext dbContext)
        {
            ...
            {
                    var idUzytkownika1 = dbContext.AppUsers.Where(u => u.UserName == "autor1@portal.pl").FirstOrDefault().Id;
                    for (int j = 0; j <= 4; j++) //teksty autora1
                    {
                        var tekst = new Models.Text()
                        {
                            Title = "Tytuł" + i.ToString() + j.ToString(),
                            Summary = "Streszczenie tekstu o tytule Title" +
                       i.ToString() + j.ToString(),
                            Keywords = "tag" + j.ToString() + ", tag" + (i +
                       j).ToString(),
        ...
        private static void SeedOpinions(ApplicationDbContext dbContext)
        {
            ...
            {
                var idUzytkownika1 = dbContext.AppUsers.Where(u => u.UserName == "autor2@portal.pl").FirstOrDefault().Id;
